	...
	<view class="container">
		...
		<view>
			<block v-for="(item,index) in dataList" :key="index">
				<fui-collapse :index="index" :current="item.current" :disabled="item.disabled" @click="change">
					<template v-slot:title>
						<fui-list-cell :hover="!item.disabled">{{item.name}}</fui-list-cell>
					</template>
					<template v-slot:content>
						<view class="fui-content">{{item.intro}}</view>
					</template>
				</fui-collapse>
			</block>
		</view>

		<view class="fui-title">禁用，无箭头</view>
		<view>
			<block v-for="(item,index) in dataList2" :key="index">
				<fui-collapse :animation="false" :index="index" :current="item.current" :disabled="item.disabled" :arrow="false" @click="change2">
					<template v-slot:title>
						<fui-list-cell :hover="!item.disabled">{{item.name}}</fui-list-cell>
					</template>
					<template v-slot:content>
		...
		<view class="fui-title">手风琴效果</view>
		<view>
			<block v-for="(item,index) in dataList" :key="index">
				<fui-collapse :index="index" :current="current" :disabled="item.disabled" @click="change3">
					<template v-slot:title>
						<fui-list-cell :hover="!item.disabled">{{item.name}}</fui-list-cell>
					</template>
					<template v-slot:content>
		...
		<view class="fui-title">手风琴效果，固定内容高度</view>
		<view>
			<block v-for="(item,index) in dataList3" :key="index">
				<fui-collapse :index="index" :current="current2" height="120rpx" bdBgColor="#fff"  @click="change4">
					<template v-slot:title>
