<template>
	<view class="container">
		...
		<view class="fui-searchbox">
			<view class="fui-search-input" @tap="search">
				<icon type="search" :size='15' color='#999'></icon>
				<text class="fui-search-text">搜索</text>
			</view>
		</view>
		<view class="fui-list-cell fui-cell-last" hover-class="fui-cell-hover" :hover-start-time="150">
			<image src="/static/images/news/2.jpg" class="fui-img"></image>
			<view class="fui-name">新的朋友</view>
		</view>
		<!--searchbox-->
		<!--联系人列表-->
		<block v-for="(list,index) in lists" :key="index">
			<fui-sticky :recalc="1" :scrollTop="scrollTop" stickyHeight="66rpx" :index="index" @change="stickyChange">
				<!--tips:sticky组件中最好不要嵌套其他自定义组件-->
				<template v-slot:header>
					<view class="fui-list-cell-divider">
						{{list.letter=="well"?"#":list.letter}}
					</view>
				</template>
				<template v-slot:content>
					<view class="fui-list-cell" :class="{'fui-cell-last':last(list.data,index2)}" hover-class="fui-cell-hover"
					 :hover-start-time="150" @tap="detail" v-for="(item,index2) in list.data" :key="index2">
						<image :src="'/static/images/news/'+((index2%2===0 && index2!==0)?'avatar_1.jpg':'avatar_2.jpg')" class="fui-img"></image>
						<view class="fui-name">{{item.name}}</view>
					</view>
				</template>
			</fui-sticky>
		</block>
		<!--联系人列表-->
		<view class="fui-footer" v-if="lists.length">120位联系人</view>
		<view class="fui-indexed-list-bar" :style="{height:indexBarHeight+'px'}">
			<view @touchstart.stop="touchStart" @touchmove.stop.prevent="touchMove" @touchend.stop="touchEnd" @touchcancel.stop="touchCancel"
			 v-for="(items,index3) in lists" :key="index3" class="fui-indexed-list-text" :style="{height:indexBarItemHeight+'px'}">
