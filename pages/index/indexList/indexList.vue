	...
	<view class="page">
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
			<view class="search-bar">
				...
				<fui-button width="120rpx" height="58rpx" :size="24" shape="circle"  @click="searchCity">搜索</fui-button>
			</view>
			<view class="fui-list search-result" v-if="inputShowed">
				<view class="fui-list-cell" hover-class="fui-list-cell-hover" v-for="(item,index) in searchResult" :key="index"
				 @tap="selectCity" :data-name="item" :hover-stay-time='150'>
					<view class="fui-list-cell-navigate">
			...
			<view v-if="!inputShowed">
				<view class="fui-list city-list">
					<block v-for="(list,index) in lists" :key="index" v-if="list.data[0]">
						<view class="fui-list-cell-divider" :id="list.letter">
							{{list.letter}}
						</view>
						<view class="fui-list-cell" hover-class="fui-list-cell-hover" v-for="(item,index2) in list.data" :key="index2"
						 @tap="selectCity" :data-name="item.name" :hover-stay-time='150'>
							<view class="fui-list-cell-navigate" :class="[list.data.length-1==index2?'last':'']">
		...
		<view class="fui-indexed-list-bar" :style="{height:indexBarHeight+'px'}" @touchstart="touchStart" @touchmove.stop="touchMove"
		 @touchend.stop="touchEnd" @touchcancel.stop="touchCancel" v-if="!inputShowed">
			<text v-for="(items,index)  in lists" :key="index" class="fui-indexed-list-text" :style="{height:indexBarItemHeight+'px'}">
				{{items.letter=="well"?"#":items.letter}}
			</text>
		</view>
		<view class="fui-indexed-list-alert" v-if="touchmove && lists[touchmoveIndex].letter">
...
<style>
	...
	.fui-indexed-list-text {
		font-size: 22rpx;
	}

	.fui-indexed-list-bar.active {
		background-color: rgb(200, 200, 200);
	}

	.fui-indexed-list-alert {
		position: absolute;
		z-index: 20;
		width: 120rpx;
		height: 120rpx;
