<template>
	<view class="container">
		...
		<view class="fui-cells">
			<textarea class="fui-textarea" placeholder="我希望加入XXXX功能，因为XX情况下，会出现XX异常" maxlength="500" placeholder-class="phcolor"
			 v-model="desc" />
			<view class="textarea-counter">{{descNum}}/500</view>
	    </view> 
	    <view class="title top64">联系邮箱</view>
	    <input class="fui-input"  placeholder-class="phcolor" placeholder="请输入邮箱" v-model="email" maxlength="20"></input>
	     <view class="fui-ptop">
			 <fui-button :type="btnType" shadow height="88rpx" shape="circle" @click="submit">提交反馈</fui-button>
	    </view>
	</view>
	...
	export default {
		...
		methods: {
			submit(){
				...
				if (!checkRes) {
					this.fui.request("/Home/AddFeedBack","POST",{
						Name: this.email,
						Subject: "问题反馈",
						Email: this.email,
						Message: this.desc
