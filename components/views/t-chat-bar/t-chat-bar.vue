<template>
	<view @touchmove.stop.prevent>
		<view class="fui-reply-tabbar" :style="{ paddingBottom: `${keyboardHeight}px` }">
			<view class="fui-chat-tabbar">
				<view class="fui-icon fui-icon-im_more fui-icon-mr" @tap="showKeyBoard(3)"></view>
				<view class="fui-icon fui-icon-im_face fui-icon-mr" @tap="showKeyBoard(2)"></view>
				<!-- v-show="showIndex != 2" -->
				<!-- <view class="fui-icon fui-icon-im_keyboard  fui-icon-mr" @tap="showKeyBoard(1)" v-show="showIndex == 2"></view> -->

				<textarea
					hold-keyboard
					auto-height
					class="fui-chat-input"
					@input="inputReply"
					:fixed="true"
					:adjust-position="false"
					maxlength="300"
					:value="content"
					:show-confirm-bar="false"
					placeholder-class="fui-phcolor"
					@tap.stop="showKeyBoard(1)"
					ref="textarea"
					v-show="!isVoice"
				></textarea>
				<view class="fui-chat-voice_btn" hover-class="fui-opcity" :hover-stay-time="150" v-if="isVoice">按住 说话</view>
				<view class="fui-send-box">
					<view class="fui-icon fui-icon-im_voice" v-if="!isVoice && !content" @tap="switchVoice"></view>
					<view class="fui-icon fui-icon-im_keyboard" v-if="isVoice && !content" @tap="switchInput"></view>
					<view class="fui-btn-send" v-if="content" :style="{ color: color }" hover-class="fui-opcity" :hover-stay-time="150" @taps="hideKeyboard">{{ sendText }}</view>
				</view>
			</view>
			<view class="fui-reply-more">
				<view class="fui-face-box" v-if="showIndex == 2">
					<scroll-view scroll-y :style="{ height: replyContainerH + 'px' }">
						<view class="fui-face-scroll">
							<view class="fui-face-img_box" v-for="(item, index) in faceList" :key="index" @tap="inputFace(index)">
							</view>
						</view>
					</scroll-view>
					<view class="fui-delete-face_box">
						<view class="fui-delete-keybord" :class="{ 'fui-opcity': !content }"><text class="fui-icon fui-icon-delete_keybord"></text></view>
					</view>
				</view>
				<view class="fui-more-box" :style="{ height: replyContainerH + 'px' }" v-if="showIndex == 3">
					<view class="fui-more-item" hover-class="fui-opcity" :hover-stay-time="150">
						<view class="fui-more-icon"><view class="fui-icon fui-icon-photo"></view></view>
						<text class="fui-more-text">照片</text>
					</view>
					<view class="fui-more-item" hover-class="fui-opcity" :hover-stay-time="150">
						<view class="fui-more-icon"><view class="fui-icon fui-icon-video"></view></view>
						<text class="fui-more-text">视频通话</text>
					</view>
					<view class="fui-more-item" hover-class="fui-opcity" :hover-stay-time="150">
						<view class="fui-more-icon"><view class="fui-icon fui-icon-at"></view></view>
						<text class="fui-more-text">@好友</text>
					</view>
					<view class="fui-more-item" hover-class="fui-opcity" :hover-stay-time="150">
						<view class="fui-more-icon"><view class="fui-icon fui-icon-location"></view></view>
						<text class="fui-more-text">定位</text>
					</view>
				</view>
			</view>
			<view class="fui-inner-mask" v-if="isLocked"></view>
		</view>
		<view
			class="fui-reply-mask"
			v-if="mask"
			:class="{ 'fui-box-show': showIndex, 'fui-mask-show': showIndex && !maskOpacity }"
			@touchmove.stop.prevent
			@tap="hideKeyboard"
		></view>
	</view>
</template>

<script>
//聊天栏 回复栏
const emoji = require('@/utils/emoji.js');
export default {
	name: 'tChatBar',
	props: {
		//禁用聊天栏
		isLocked: {
			type: Boolean,
			default: false
		},
		//是否显示遮罩
		mask: {
			type: Boolean,
			default: true
		},
		//true表示遮罩透明
		maskOpacity: {
			type: Boolean,
			default: true
		},
		sendText: {
			type: String,
			default: '发送'
		},
		color: {
			type: String,
			default: '#5677fc'
		}
	},
	created() {
		//键盘高度监听
		this.faceList = emoji.en;
		let safeH = this.fui.isPhoneX() ? 34 : 0;
		uni.onKeyboardHeightChange(res => {
			let h = res.height - safeH;
			this.keyboardHeight = h > 0 ? h : 0;
			//去除 完成那一栏高度影响
			setTimeout(() => {
				if (this.showIndex == 1 && this.keyboardHeight != 0) {
					this.replyContainerH = this.keyboardHeight;
				}
			}, 100);
		});
	},
	data() {
		return {
			showIndex: 0, //1-键盘 2-表情 3-其他
			keyboardHeight: 0,
			replyContainerH: uni.upx2px(500),
			faceList: [],
			content: '',
			isVoice: false
		};
	},
	methods: {
		hideKeyboard() {
			//隐藏键盘
			this.showIndex = 0;
			// uni.hideKeyboard();
		},
		showKeyBoard(index) {
			if (this.showIndex == index) return;
			this.showIndex = index;
			this.isVoice = false;
			// if(index==1){
			// 	this.$refs.textarea.focus();
			// }
			// setTimeout(function() {
			// 	if(index==1){
			// 		this.$refs.textarea.focus();
			// 	}
			// }, 150);
		},
		inputReply(e) {
			this.content = e.detail.value;
		},
		inputFace(index) {
			let face = emoji.cn[index];
			this.content = this.content + face;
		},
		switchVoice() {
			this.isVoice = true;
			this.showIndex = 0;
		},
		switchInput() {
			this.isVoice = false;
		}
	}
};
</script>

<style lang="scss" scoped>
@font-face {
	font-family: 'ticons';
	src: url(data:application/font-woff;charset=utf-8;base64,)
		format('woff');
	font-weight: normal;
	font-style: normal;
}

.fui-icon {
	font-family: ticons;
	font-size: 24px;
	font-weight: normal;
	font-style: normal;
	line-height: 1;
	display: inline-block;
	text-decoration: none;
	-webkit-font-smoothing: antialiased;
}

.fui-icon-at:before {
	content: '\eb90';
}

.fui-icon-location:before {
	content: '\eb95';
}

.fui-icon-im_more:before {
	content: '\eb96';
}

.fui-icon-im_face:before {
	content: '\eb97';
}

.fui-icon-link:before {
	content: '\eb98';
}

.fui-icon-im_voice:before {
	content: '\eb99';
}

.fui-icon-photo:before {
	content: '\eb9a';
}

.fui-icon-redpacket:before {
	content: '\eb9b';
}

.fui-icon-im_keyboard:before {
	content: '\eb9c';
}

.fui-icon-principal:before {
	content: '\eb9d';
}

.fui-icon-video:before {
	content: '\eba2';
}
.fui-icon-delete_keybord:before {
	content: '\e7b8';
}

.fui-reply-tabbar {
	width: 100%;
	position: fixed;
	left: 0;
	bottom: 0;
	display: flex;
	flex-direction: column;
	box-sizing: border-box;
	z-index: 999;

	.fui-chat-tabbar {
		width: 100%;
		background-color: $uni-bg-color-grey;
		display: flex;
		align-items: flex-end;
		padding: 16rpx 24rpx;
		box-sizing: border-box;

		&::after {
			content: ' ';
			position: absolute;
			top: 0;
			right: 0;
			left: 0;
			border-top: 1rpx solid $uni-text-color-placeholder;
			-webkit-transform: scaleY(0.5);
			transform: scaleY(0.5);
		}

		.fui-chat-input {
			flex: 1;
			/* #ifndef MP-WEIXIN */
			min-height: 46rpx;
			padding: $uni-spacing-col-base 0;
			/* #endif */
			/* #ifdef MP-WEIXIN */
			min-height: 78rpx;
			/* #endif */
			background-color: $uni-bg-color;
			border-radius: 8rpx;
			margin: 0;
		}

		.fui-chat-voice_btn {
			flex: 1;
			height: 78rpx;
			border-radius: 8rpx;
			background-color: #fff;
			display: flex;
			align-items: center;
			justify-content: center;
			font-weight: bold;
		}

		.fui-icon-mr {
			margin-right: 28rpx;
		}

		.fui-icon-ml {
			margin-left: 28rpx;
		}

		.fui-icon {
			margin-bottom: 13rpx;
		}
		.fui-send-box {
			width: 80rpx;
			display: flex;
			align-items: center;
			justify-content: flex-end;
		}

		.fui-btn-send {
			flex-shrink: 0;
			padding: 22rpx 0;
			font-size: 30rpx;
			line-height: 30rpx;
		}
	}

	.fui-reply-more {
		width: 100%;
		position: relative;

		&::after {
			content: ' ';
			position: absolute;
			top: 0;
			right: 0;
			left: 0;
			border-top: 1rpx solid $uni-text-color-placeholder;
			-webkit-transform: scaleY(0.5);
			transform: scaleY(0.5);
		}

		.fui-face-box {
			width: 100%;
			position: relative;
			background-color: $uni-bg-color-hover;

			.fui-face-scroll {
				width: 100%;
				padding: $uni-spacing-row-lg 24rpx 80rpx;
				box-sizing: border-box;
				display: flex;
				flex-wrap: wrap;

				.fui-face-img_box {
					width: 14.2857%;
					height: 100rpx;
					display: flex;
					align-items: center;
					justify-content: center;

					.fui-face-img {
						width: 60rpx;
						height: 60rpx;
					}
				}
			}

			.fui-delete-face_box {
				position: absolute;
				background-color: $uni-bg-color-hover;
				width: 108rpx;
				height: 92rpx;
				right: 20rpx;
				bottom: 0;
				z-index: 10;

				.fui-delete-keybord {
					width: 108rpx;
					height: 68rpx;
					background-color: $uni-bg-color;
					display: flex;
					align-items: center;
					justify-content: center;
					border-radius: $uni-border-radius-lg;

					.fui-icon {
						font-size: 48rpx;
					}
				}
			}
		}

		.fui-more-box {
			width: 100%;
			padding: $uni-spacing-row-lg;
			box-sizing: border-box;
			position: relative;
			display: flex;
			align-items: flex-start;
			flex-wrap: wrap;
			background-color: $uni-bg-color-hover;

			.fui-more-item {
				width: 126rpx;
				display: flex;
				align-items: center;
				flex-direction: column;
				justify-content: center;
				margin-right: 40rpx;

				&:active {
					opacity: 0.6;
				}
			}

			.fui-more-icon {
				display: flex;
				align-items: center;
				justify-content: center;
				background-color: #ffffff;
				height: 126rpx;
				width: 126rpx;
				border-radius: 32rpx;

				.fui-icon {
					font-size: 60rpx;
				}
			}

			.fui-more-text {
				font-size: 24rpx;
				line-height: 24rpx;
				color: #7e7e7e;
				padding-top: 20rpx;
				padding-bottom: 20rpx;
			}
		}
	}

	.fui-inner-mask {
		position: absolute;
		width: 100%;
		height: 100%;
		left: 0;
		top: 0;
		background-color: rgba(248, 248, 248, 0.6);
	}
}

.fui-reply-mask {
	width: 100%;
	height: 100%;
	position: fixed;
	left: 0;
	top: 0;
	z-index: 990;
	background-color: $uni-bg-color-mask;
	visibility: hidden;
	transition: all 0.3s ease;
	opacity: 0;
}

.fui-box-show {
	visibility: visible !important;
}
.fui-mask-show {
	opacity: 1;
}
</style>
